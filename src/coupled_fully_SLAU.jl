...
function coupled!(
    ...
    for face in faces_internal
       ...
       #= Hₜₗ = cells[face.owner].var[👉.Hₜ]
        ...
        ṁ = 0.5*(ρₗ*(Uₙₗ+Vnp)+ρᵣ*(Uₙᵣ-Vnm)-(1.0-Mdash)^2/c̄*(pᵣ-pₗ))
        #ṁ = 0.5*(ρₗ*Uₙₗ+ρᵣ*Uₙᵣ) + 0.5*(ρₗ*Vnp-ρᵣ*Vnm) - 0.5*(1.0-Mdash)^2/c̄*(pᵣ-pₗ)
        ṁₗ = 0.5*(ṁ+abs(ṁ))
        ṁᵣ = 0.5*(ṁ-abs(ṁ))
        ...
        Uₙ = (Wₗ/ρₗ + Wᵣ/ρᵣ)*ṁ + Uₙ_RC

        WUₗ = (Wₗ/ρₗ + Wᵣ/ρᵣ)*0.5*ρₗ
        WUᵣ = (Wₗ/ρₗ + Wᵣ/ρᵣ)*0.5*ρᵣ

        Wpₗ = p⁺
        Wpᵣ = p⁻

        diffΔp = RCdiffΔp + (Wₗ/ρₗ + Wᵣ/ρᵣ)*0.5*(1.0-Mdash)^2/c̄
   ...
   # exit()
    ...
    for cell in cells
        ...
        cell.var[👉.p] = max(cell.var[👉.p],10.0)
        cell.var[👉.T] = max(cell.var[👉.T],10.0)
